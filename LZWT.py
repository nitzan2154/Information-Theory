...
class LZW:
    def __init__(self, size=None):
        self.initial_dict_size = size if size else 128

    def transform(self, data):
        """Lempel-Ziv-Welch transform"""
        # Initialize dictionary with single characters
        dictionary = {chr(i): bin(i)[2:] for i in range(self.initial_dict_size)}
        dict_size = self.initial_dict_size
        bit_length = ceil(log2(dict_size))
        ...
        for code in data:
            pcode = p + code
            if pcode in dictionary:
                p = pcode
            else:
                padding = '0' * (bit_length - len(dictionary[p]))
                compressed_data.append(padding + dictionary[p])
                dictionary[pcode] = bin(dict_size)[2:]
                dict_size += 1
                bit_length = ceil(log2(dict_size))
                p = code
    ...
    def inverse_transform(self, compressed):
        """Lempel-Ziv-Welch reverse transform"""
        dictionary = {i: chr(i) for i in range(self.initial_dict_size)}
        dict_size = self.initial_dict_size
        bit_length = ceil(log2(dict_size))

        # Get the first code and initialize the result
        w = dictionary[int(compressed[:bit_length], 2)]
        result = w
        compressed = compressed[bit_length:]
        length = len(compressed)

        index = 0
        bit_length = ceil(log2(dict_size + 1))
        while (index + bit_length) <= length:
            k = int(compressed[index:(index + bit_length)], 2)
            if k in dictionary:
                entry = dictionary[k]
            elif k == dict_size:
                # Special case: k equals the current dict size, entry is w + w[0]
                entry = w + w[0]
            else:
                raise ValueError(f"Invalid compressed k: {k}")

            result += entry

            # Add new entry to the dictionary (w + entry[0])
            dictionary[dict_size] = w + entry[0]
            dict_size += 1
            index += bit_length
            bit_length = ceil(log2(dict_size + 1))
            # Update w to be the current entry
            w = entry
...
def main():
    ...
    compressed = lzw.transform(data)
    print("Compressed ratio:", len(compressed) / (8 * len(data)))

    decompressed = lzw.inverse_transform(compressed)
